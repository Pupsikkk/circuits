...
const Examples = ({ importContent }: Props) => {
  ...
  // Scale coordinates based on window size
  function scaleHeight(height: number) {
    return (height / 965) * windowSize.height;
  }
  function scaleWidth(width: number) {
    return (width / 1920) * windowSize.width;
  }

  // Example circuit/arrow objects
  const OssCirc = [
    {
